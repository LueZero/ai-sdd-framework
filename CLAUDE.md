...
每個階段有對應的 Skill（`/01-init-project` 至 `/08-change-request`）和專屬 Subagent（`sdd-pm`, `sdd-architect`, `sdd-developer`, `sdd-reviewer`）。
...
- 規格變更：參照 @sdd/workflows/change-request.md
...
- 編碼規範：@.claude/rules/coding-standards.md
- SDD 流程規則：@.claude/rules/sdd-process.md
- 安全準則：@.claude/rules/security.md
